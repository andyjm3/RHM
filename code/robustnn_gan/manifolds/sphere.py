"""
Modified from geoopt and add ehess2rhess (without projection) for Sphere 
"""
...
from geoopt.utils import size2shape, broadcast_shapes
...
class Sphere(Manifold):
    ...
    def ehess2rhesspreproj(self, x: torch.Tensor, egrad: torch.Tensor, ehess: torch.Tensor, u: torch.Tensor):
        return  ehess - (x * egrad).sum(dim=-1, keepdim=True) * u
